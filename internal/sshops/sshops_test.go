...
func TestServiceStatuses_MissingKey(t *testing.T) {
	if _, err := ServiceStatuses("192.0.2.1", "user", badKey, []string{"nginx"}); err == nil {
		t.Fatal("expected error")
	}
}

func TestServiceControl_MissingKey(t *testing.T) {
	if err := ServiceControl("192.0.2.1", "user", badKey, "nginx", "restart"); err == nil {
		t.Fatal("expected error")
	}
}

func TestJournalLines_MissingKey(t *testing.T) {
	if _, err := JournalLines("192.0.2.1", "user", badKey, "nginx", 10); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestStreamJournal_MissingKey(t *testing.T) {
	...
	StreamJournal("192.0.2.1", "user", badKey, "nginx", rr, httptest.NewRequest("GET", "/", nil))
...
func TestStreamJournal_NoFlusher(t *testing.T) {
	w := &plainWriter{}
	StreamJournal("192.0.2.1", "user", badKey, "nginx", w, httptest.NewRequest("GET", "/", nil))
	if w.written {
